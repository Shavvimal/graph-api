...
def convert_response_to_string(response: Union[str, Dict[str, Any], List[str], List[Dict[str, Any]]]) -> str:
    """
...
def process_context_data(context_data: Union[str, List[pd.DataFrame], Dict[str, pd.DataFrame]]):
    if isinstance(context_data, str):        
        ...
        # If it's a dictionary, check if each value is a string or DataFrame
        return {key: value if isinstance(value, str) else value.to_dict(orient="records")
